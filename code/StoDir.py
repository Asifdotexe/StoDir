...
def fetch_data(ticker: str,
               ...
               history_start: str = "1990-01-01") -> pd.DataFrame:
    """
    ...
    - The function saves the plot in a directory named 'plots'
    with the filename as '{ticker}_closing_price.png' and
    '{ticker}_candlestick_chart.png'.
...
def add_features(data: pd.DataFrame, *horizons) -> pd.DataFrame:
    ...
    if the closing price of the next day is higher than
    the current closing price, and 0 otherwise.
    3. For each horizon in the list of horizons (2, 5, 60, 250, 1000),
    it calculates the rolling average of the closing price over that horizon
    and divides it by the current closing price to create
    a new feature with the name '{horizon}_day'.
...
def predict_next_day(model, data):
    ...
    Returns:
    - str: The predicted direction of the next day's stock price. Returns "up"
    if the probability of an upward movement is greater than 0.6,
    otherwise returns "down".

    The function selects the most recent data from the input DataFrame,
    extracts relevant features, and uses the trained model to
    predict the probabilities of an upward or downward movement.
    It then compares the probabilities to a threshold (0.6)
...
def main():
    parser = argparse.ArgumentParser(
        description='Predict the next day stock price movement.')
    parser.add_argument('ticker', type=str, help='Stock ticker symbol')
    parser.add_argument('--horizons', type=int, nargs='*',
                        help='List of horizons to include as features')
